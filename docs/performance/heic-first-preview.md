...
| Issue #5 候选 | 1272.8, 1114.9, 1123.3, 1131.1, 1204.9 | 1131.1 ms | 851.1, 691.3, 703.4, 620.9, 682.5 | 691.3 ms |

冷缓存中位数降低 36.8%，超过验收要求的 30%。热缓存 5 次上传均未再次请求解码器模块。最终报告的冷/热最大帧间隔为 50.1/50.0ms，最大 Long Task 为 60/61ms。

### 口径敏感性

- 将 `pointerdownDelayMs` 设为 0、不留上传意图预热时间时，候选冷启动中位数为 1602.8ms，相对同一基线改善 10.4%。该控制用于明确优化对文件选择器停留时间的依赖，不作为约定的“选择完成到首张可编辑预览”验收口径。
- 额外 10 次冷/热压力复测中，最大帧间隔分别为 66.7/100.1ms，最大 Long Task 分别为 74/110ms，全部低于 150ms 门槛。
